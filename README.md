...
`.env`의 API 키는 서버 쪽에만 남아 있으며 브라우저로 전달되지 않습니다.
이 도구는 `127.0.0.1` 로컬 주소에서만 열립니다.
...
음악과 이미지 생성에는 API 사용 비용이 발생할 수 있습니다. 생성한 WAV, PNG, MP4는 각각 웹 화면에서 내려받을 수 있으며 `workspace/web-jobs/` 아래에도 보관됩니다.
이 명령은 YouTube 업로드를 실행하지 않고, 일일 스케줄러나 `launchd` 작업도 활성화하지 않습니다.
